...
void RigidBodyManager :: addTBar(TwBar *bar)
{
	...
	TwAddVarRW(bar, "Coefficient of Restitution", TW_TYPE_FLOAT, &coeffRestitution, "");
...
{
	int size = broad_collision_pairs.size();
...
float RigidBodyManager :: EPA_GJK(CollisionPair pair, glm::vec3 &coll_normal, glm::vec3 &pointA, glm::vec3 &pointB, std::vector<glm::vec3> &a_tri, std::vector<glm::vec3> &b_tri, float dt)
{
	std::vector<glm::vec3> simplex;
	...
	if(epaGJK(pair, sim))
	{
		...
		glm::vec3 padot = pair.body1->velocity + glm::cross(pair.body1->ang_velocity, pointA - pair.body1->centre_of_mass);
		glm::vec3 pbdot = pair.body2->velocity + glm::cross(pair.body2->ang_velocity, pointB - pair.body2->centre_of_mass);





		float vrel_before = glm::dot(n, padot - pbdot);
		...
		padot = pair.body1->velocity + glm::cross(pair.body1->ang_velocity, pointA - pair.body1->centre_of_mass);
		pbdot = pair.body2->velocity + glm::cross(pair.body2->ang_velocity, pointB - pair.body2->centre_of_mass);
		



		float vrel_after = glm::dot(n, padot - pbdot);
...
void RigidBodyManager :: backtrack(CollisionPair &pair, float dt, float threshold, float distance, std::vector<SupportPoint> &new_sim, glm::vec3 &new_contact_point, glm::vec3 &new_contact_normal)
{
	std::vector<SupportPoint> sim;
	glm::vec3 coll_point, coll_normal;
	float dist;

	std::vector<SupportPoint> best_sim = new_sim;
	glm::vec3 best_coll_point = new_contact_point;
	glm::vec3 best_coll_normal = new_contact_normal;
	float best_dist = distance;
	RigidBody best_At = *pair.body1;
	RigidBody best_Bt = *pair.body2;

	RigidBody *A = pair.body1;
	RigidBody *B = pair.body2;
	glm::vec3 pa = A->position;
	glm::vec3 pb = B->position;
	glm::vec3 va = A->velocity;
	glm::vec3 vb = B->velocity;

	glm::vec3 a_pos_t0 = pa - va*dt;
	glm::vec3 b_pos_t0 = pb - vb*dt;

//	glm::vec3 wa = A->ang_velocity;
//	glm::vec3 wb = A->ang_velocity;
//	glm::mat3 wa_mat = A->makeAngVelMat(wa);
//	glm::mat3 wb_mat = B->makeAngVelMat(wb);
	...
	RigidBody B0 = *B;
/*
	A0.position = a_pos_t0;
	B0.position = b_pos_t0;

	A0.translation_mat = glm::translate(glm::mat4(), A0.position);
	B0.translation_mat = glm::translate(glm::mat4(), B0.position);

	

	A0.model_mat = A0.translation_mat * glm::mat4(A0.rotation_mat) * A0.scale_mat;
	B0.model_mat = B0.translation_mat * glm::mat4(B0.rotation_mat) * B0.scale_mat;

	A0.transformVertices();
	B0.transformVertices();
	*/
	glm::vec3 a_pos_change = A0.position - A0.prev_position;
	glm::vec3 b_pos_change = B0.position - B0.prev_position;

	A0.position = A0.prev_position;
	B0.position = B0.prev_position;
	A0.model_mat = A0.prev_model_mat;
	B0.model_mat = B0.prev_model_mat;
	A0.transformVertices();
	B0.transformVertices();

	

	CollisionPair t0_pair (A0, B0);
	...
	CollisionPair t_pair(At, Bt);
	CollisionPair best_pair(best_At, best_Bt);

	if(epaGJK(t0_pair, sim))
	{
		cout << "lol";
	}
	...
	while(counter > 0)
	{
		...
		At.position += a_pos_change*T;
		Bt.position += b_pos_change*T;
		At.translation_mat = glm::translate(glm::mat4(), At.position);
		Bt.translation_mat = glm::translate(glm::mat4(), Bt.position);
		At.model_mat = At.translation_mat * glm::mat4(At.rotation_mat) * At.scale_mat;
		Bt.model_mat = Bt.translation_mat * glm::mat4(Bt.rotation_mat) * Bt.scale_mat;
		At.transformVertices();
		Bt.transformVertices();

		if(epaGJK(t_pair, sim))
		{
			dist = EPA(t_pair, sim, coll_normal, coll_point);
			if(dist < best_dist)
			{
				best_dist = dist;
				best_sim = sim;
				best_coll_normal = coll_normal;
				best_coll_point = coll_point;
				best_At = At;
				best_Bt = Bt;
	...
	new_sim = best_sim;
	new_contact_point = best_coll_point;
	new_contact_normal = best_coll_normal;
	pair = best_pair;
...
float RigidBodyManager :: EPA_GJK_bt(CollisionPair pair, glm::vec3 &coll_normal, glm::vec3 &pointA, glm::vec3 &pointB, std::vector<glm::vec3> &a_tri, std::vector<glm::vec3> &b_tri, float dt)
{
	std::vector<glm::vec3> simplex;
	std::vector<SupportPoint> sim;
	float dist = 0.0f;
	...
	if(epaGJK(pair, sim))
	{
		...
		if(dist > threshold)
			backtrack(pair, dt, threshold, dist, sim, p, coll_normal);
